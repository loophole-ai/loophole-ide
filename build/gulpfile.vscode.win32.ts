...
function buildWin32Setup(arch: string, target: string): task.CallbackTask {
	...
	return (cb) => {
		...
		const definitions: Record<string, unknown> = {
			...
			Version: (product as typeof product & { loopholeVersion?: string }).loopholeVersion ?? pkg.version,
			RawVersion: ((product as typeof product & { loopholeVersion?: string }).loopholeVersion ?? pkg.version).replace(/-\w+$/, ''),
			Commit: commit,
