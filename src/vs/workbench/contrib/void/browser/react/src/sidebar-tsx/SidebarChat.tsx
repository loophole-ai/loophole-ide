					...
					>
						...
						>
							...
							{selection.type === 'File' && selection.state.wasAddedAsCurrentFile && messageIdx === undefined && currentURI?.fsPath === selection.uri.fsPath ?
								<span className={`text-[8px] 'loophole-opacity-60 text-loophole-fg-4`}>
									{`(Current File)`}
								</span>
								: null
							}
				...
				<div className={`flex items-center w-full gap-x-2 overflow-hidden justify-between ${isRejected ? 'line-through' : ''}`}>
					...
					<div className="flex items-center gap-x-2 flex-shrink-0">
						...
						{isError && <AlertTriangle
							className='text-loophole-warning opacity-90 flex-shrink-0'
							size={14}
							data-tooltip-id='loophole-tooltip'
...
const LintErrorChildren = ({ lintErrors }: { lintErrors: LintErrorItem[] }) => {
	return <div className="text-xs text-loophole-fg-4 opacity-80 border-l-2 border-loophole-warning px-2 py-0.5 flex flex-col gap-0.5 overflow-x-auto whitespace-nowrap">
...
const BottomChildren = ({ children, title }: { children: React.ReactNode, title: string }) => {
	...
	return (
		<div className="w-full px-2 mt-0.5">
			...
			>
				<div className="overflow-x-auto text-loophole-fg-4 opacity-90 border-l-2 border-loophole-warning px-2 py-0.5">
					{children}
				</div>
			</div>
		</div>
