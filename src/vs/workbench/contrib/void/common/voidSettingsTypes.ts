...
export const subTextMdOfProviderName = (providerName: ProviderName): string => {
	...
	if (providerName === 'appleFoundationModels') return 'Loophole can auto-detect two models here: **system** (fully on-device, no network) and **pcc** (offloaded to Apple\'s [Private Cloud Compute](https://security.apple.com/blog/private-cloud-compute/) for larger requests, under the same privacy guarantees as on-device). On macOS 27+ these come from the built-in `fm serve`; on macOS 26 and earlier from [maclocal-api](https://github.com/scouzi1966/maclocal-api) (`afm` on port 9999, install via Homebrew or `pip install macafm`, model id `foundation`). For MLX models on the same stack, run `afm mlx -m <model>` separately or use the **MLX** provider.'
