...
export class EditorGroupWatermark extends Disposable {
	...
	private render(): void {
		...
		const update = async () => {
			...
			// Loophole - if the workbench is empty, show open
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				openFolderButton.root.classList.add('loophole-openfolder-button')
				...
				openSSHButton.root.classList.add('loophole-openssh-button')
				...
				if (recentlyOpened.length !== 0) {
					...
					voidIconBox.append(
						...recentlyOpened.map((w, i) => {
							...
							const linkSpan = $('span');
							linkSpan.classList.add('loophole-link')
				...
				// button3.textContent = 'Loophole Settings'
